...
int main() {
  ...
  printf("%c\n", gender);
  printf("%d\n", (int)gender); // 형변환 안해도 동일하게 77 출력(ASCII 값)

  char num = 127;
  printf("%c %d\n", num, num);

  // C언어는 기본적으로 bool 타입이 포함되어 있지 않음, 참/거짓을 구분하는 데이터 타입, C언어 1/0으로 기본적 판별함
  bool is_adult = true;

  // sizeof() : 데이터의 크기를 확인할 때 사용
  printf("int 형의 크기 : %lu 바이트\n", sizeof(int)); // lu : long unsigned int

  printf("주소의 크기 : %lu 바이트\n", sizeof(int *));
  printf("주소의 크기 : %lu 바이트\n", sizeof(char *));
  printf("주소의 크기 : %lu 바이트\n", sizeof(void *));

  // 열거형 : 특정한 값을 다른 이름으로 명명할 때 사용됨
  enum Color { RED = 100, ORANGE, YELLOW, GREEN, BLUE};
  enum Color color = BLUE;
  printf("Color : %d]n", color);

  // 구조체 : 여러 개의 변수를 하나의 그룹으로 묶는 데 사용
  struct Member {
    int id;
    char name[20];
    int age;
